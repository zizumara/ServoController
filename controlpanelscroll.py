...
class PCA9685Panel(Frame):
    ...
    def __init__(self, configFile, i2cBaseAddr, updateFreqHz, parent=None, presetFile=None):
        ...
        bg = self.BG_COLOR1
        self.sliderDict = {}
        sliderCanvas = Canvas(self)
        scrollBar = Scrollbar(self, orient='vertical', command=sliderCanvas.yview)
        sliderFrame = Frame(sliderCanvas)
        sliderFrame.bind('<Configure>',
                         lambda e: sliderCanvas.configure(scrollregion=sliderCanvas.bbox('all'))
                        )
        sliderCanvas.create_window(0, 0, window=sliderFrame, anchor='nw')
        sliderCanvas.configure(yscrollcommand=scrollBar.set)
        for (channel, servo) in self.controller.servoDict.items():
            slider = PCA9685ServoSlider(sliderFrame, labelText=f'Channel {channel}: {servo.name}',
            ...
            if bg == self.BG_COLOR1:
                ...
                bg = self.BG_COLOR1
        sliderCanvas.pack(side=LEFT, fill=BOTH, expand=True)
...
if __name__ == '__main__':
    ...
    parser.add_argument('-c', '--config', required=True, help='path to servo configuration file')
    parser.add_argument('-p', '--presets', required=True, help='path to servo presets file')
    ...
    root = Tk()
    panel = PCA9685Panel(args['config'], I2C_BASE_ADDR, UPDATE_FREQ_HZ, parent=root,
